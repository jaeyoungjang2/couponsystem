...
#
# 사용:
#   ./scripts/load/v9/run.sh             # 두 시나리오 모두 실행 (워밍업 + 본 측정)
#   ./scripts/load/v9/run.sh policy      # ① 발급 API 쿠폰 정보 조회 요청 급증만
#   ./scripts/load/v9/run.sh sellout     # ② 매진 후 새로고침만
#   ./scripts/load/v9/run.sh --once policy # 워밍업 생략
#
...
run_policy_once() {
  ...
  ./scripts/load/v9/reset.sh
  local coupon_id
  coupon_id=$(./scripts/load/v9/create_issue_policy_coupon.sh)
  reset_metrics
  # threshold (p99<200) 깨지는 게 4-0 ~ 4-1b 에서 정상. SWR (4-1c) 에서 통과.
  k6 run -e COUPON_ID="$coupon_id" scripts/load/v9/coupon_burst.js || true
...
run_sellout_once() {
  ...
  ./scripts/load/v9/reset.sh
  local coupon_id
  coupon_id=$(./scripts/load/v9/create_small_coupon.sh)
  COUPON_ID="$coupon_id" ./scripts/load/v9/sell_out.sh
  reset_metrics
  # 4-2 적용 후엔 threshold (p99<100) 통과해야 한다.
  k6 run -e COUPON_ID="$coupon_id" scripts/load/v9/post_sellout_refresh.js || true
  printf '\n\033[1;33m카운터: \033[0m%s\n' "$(read_metrics)"
}
